...
const  HeaderCategoroySelector = async () => {
  ...
  return (
    <div className='relative inline-block group'>
        ...
        <div className='absolute top-full left-0 pt-2 opacity-0 invisible peer-hover:opacity-100 peer-hover:visible hover:opacity-100 hover:visible transition-all duration'>
            <div className='bg-white w-64 rounded-lg shadow-xl border border-gray-100 overflow-hidden'>
                <div className='py-2'>
                    {categories && categories.map((category) => (
                        ...
                        prefetch
                        className='block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors duration-100'>{category.title}</Link>
                    ))}
                </div>
            </div>
        </div>
...
export default HeaderCategoroySelector